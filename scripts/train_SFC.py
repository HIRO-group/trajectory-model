...
def train_model(args, model, X, Y): 
    ...
    save_address = generate_address(args=args, acc=val_loss_acc[1], loss=val_loss_acc[0])
    model.save_weights(save_address)
    print(f'Saved model to disk.')
    
    if args.show_loss_function:
...
def load_model(args, model, X, Y):
    ...
    prediction = model.predict({"trajectory": X_train_traj[0, :, :7][None, :, :], "properties": X_train_prop[0, :][None, :],})[0][0]
