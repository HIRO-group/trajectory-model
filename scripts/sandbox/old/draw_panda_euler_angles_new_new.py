...
def get_rot_panda_traj(file_name):
    ...
    panda_traj = rotate_traj_and_eulerize(panda_traj)
    return panda_traj
...
def plot_euler_angle_for_a_traj(traj, axs, color, alpha, linewidth, start=0, end=-1):
    euler_angles = traj[start:end, 3:6]
...
def plot_euler_angles_rot(rot_panda_trajs):
    ...
    # # rot_panda_traj = get_rot_panda_traj(task_4)
    # # plot_euler_angle_for_a_traj(rot_panda_traj, axs, 'limegreen', alpha=0.5, linewidth=2, start=2606, end=3652)
    ...
    print(rot_panda_traj[0, 3:6])
    print(rot_panda_traj[2000, 3:6])
...
def rotate_traj_and_eulerize(panda_traj):
    euler_angles = get_euler_angles(panda_traj)
    for i in range(0, 3):
        euler_angles[:, i] -= euler_angles[0, i]

    panda_traj[:, 3:6] = euler_angles
    return panda_traj
    ...
    for task in Tasks.all:
        for fn in task:
            ...
            if not good_data(panda_traj):
                continue
            rot_panda_traj = rotate_traj_and_eulerize(panda_traj)
