...
from trajectory_model.informed_sampler.constants import EMBED_DIM_ORIENTATION, MAX_NUM_WAYPOINTS
...
if __name__ == "__main__":
    ...
    model = OrientationSampler(max_num_waypoints=MAX_NUM_WAYPOINTS, waypoints_embed_dim=EMBED_DIM_ORIENTATION)
    ...
    if fit_model:
        ...
        if save == 'y':
            model.save_weights(f'weights/orientation_sampler/{ctime_str()}_acc_{round(acc_val, 2)}_loss_{round(loss_val, 2)}_data_num_{training_data_num}_epochs_{epochs}.h5')
        ...
        model.build((None, MAX_NUM_WAYPOINTS, EMBED_DIM_ORIENTATION))
        model.load_weights("weights/orientation_sampler/acc_0.85_loss_0.01_data_num_165_epochs_400.h5")
